...
mkdir -p ${target_dir}
for d in $domains; do
  target_crt=${target_dir}/${d}.crt
  if [[ ${force} ]] || [[ ! -e ${target_crt} ]] || [[ -e ${target_crt} && $(openssl x509 -in ${target_crt} -noout -enddate -checkend ${checkend} | grep 'will expire') ]];then
    /usr/lib/acme/client/acme.sh --installcert --home ${source_dir} -d ${d} --cert-file ${target_crt} --key-file ${target_dir}/${d}.key --fullchain-file ${target_dir}/${d}.pem --reloadcmd "$reloadcmd" | logger -t "${LOG_TAG}"
  fi
done