    ...
    return Object.fromEntries(
      Object.entries(repositories).map(([name, url]) => [
        ...
        String(url)
          ...
          .replaceAll("{release_rel}", openwrtVersion === 'snapshots' ? openwrtVersion : ("releases/" + openwrtVersion + ""))
          .replaceAll("{package_rel}", openwrtVersion === 'snapshots' ? "packages" : ("releases/" + "packages-" + openwrtBranch))
          .replaceAll("{arch_packages}", String(currentDevice.arch_packages || ""))
          .trimEnd("/")+(openwrtVersion === 'snapshots' || openwrtBranch >= '25.12' ?"/packages.adb":"")
      ])
    );
  }
